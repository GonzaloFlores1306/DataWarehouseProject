...
BEGIN
	...
	BEGIN TRY
		...
		SELECT
			...
			END AS sls_due_dt,
			CASE WHEN sls_sales IS NULL OR sls_sales <= 0 OR sls_sales != sls_quantity * ABS(sls_price) --Negative to positive
		...
		SELECT
			...
			CASE WHEN bdate > GETDATE() THEN NULL -- Set future birthdates to NULL
				 WHEN bdate < '1924-01-01' THEN NULL -- Set past birthdates to NULL
				 ELSE bdate
			END AS bdate, 
